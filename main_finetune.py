...
from models.Newname import Mymodel
...
def main(args):
    ...
    utils.auto_load_model(
        ...
        optimizer_gn=optimizer_gn, optimizer_dt=optimizer_dt, loss_scaler=loss_scaler, model_ema=model_ema)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer_gn, optimizer_dt, device, epoch, loss_scaler,
        ...
        if args.output_dir and args.save_ckpt:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                utils.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer_gn=optimizer_gn,
                    optimizer_dt=optimizer_dt, loss_scaler=loss_scaler, epoch=epoch, model_ema=model_ema)
                utils.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer_gn=optimizer_gn,
                    optimizer_dt=optimizer_dt, loss_scaler=loss_scaler, epoch='last', model_ema=model_ema)
        if data_loader_val is not None:
            ...
            if max_accuracy < test_stats["acc1"]:
                ...
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer_gn=optimizer_gn,
                        optimizer_dt=optimizer_dt, loss_scaler=loss_scaler, epoch="best", model_ema=model_ema)
            ...
            if args.model_ema and args.model_ema_eval:
                ...
                if max_accuracy_ema < test_stats_ema["acc1"]:
                    ...
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer_gn=optimizer_gn,
                            optimizer_dt=optimizer_dt, loss_scaler=loss_scaler, epoch="best-ema", model_ema=model_ema)
